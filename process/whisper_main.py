...
# VERIFICATION : SANS PRE-TRAITEMENT 
import whisper

model = whisper.load_model("base")
result = model.transcribe("sons/Ciel mon mari.mp4")
print(result["text"])
...
model = whisper.load_model("base")
segments_dir = "processed_segments"

for seg_file in sorted(os.listdir(segments_dir)):
    if seg_file.endswith(".wav"):
        file_path = os.path.join(segments_dir, seg_file)
        result = model.transcribe(file_path)
        print(f"{seg_file} -> {result['text']}")
